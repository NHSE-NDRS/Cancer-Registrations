...
### constants
DEFAULT_HOST <- ""     ## Default name of database host machine
DEFAULT_PORT <-             ## Default port number
# DEFAULT_PORT <-             ## Port number when connecting from CAS3
DEFAULT_SID  <- ""      ## Default CAS database (schema)
DEFAULT_DRIVER <- "ojdbc7.jar"  ## Default path and filename of JDBC driver
DEFAULT_XMS  <- 1							  ## Default memory reserved for CAS data transfer in gigabytes
DEFAULT_XMX  <- 16							## Maximum memory for CAS data transfer in gigabytes
...
#' @param xms Memory reserved for data transfer in GB.  Default is 1
#' @param xmx Maximum memory allowed for data transfer in GB.  Default is 16
#'
#' @return Returns a database connection object.
#' @export
#'
#' @examples
createConnection <- function(hostname=DEFAULT_HOST , port=DEFAULT_PORT, sid=DEFAULT_SID, drv_path=DEFAULT_DRIVER, xms=DEFAULT_XMS, xmx=DEFAULT_XMX, username = NA)
{
  print(.jinit(parameters = c(paste0("-Xms",xms,"G"),paste0("-Xmx",xmx,"G")), force.init = TRUE))
  
  ## set up DB connection
  drv <- JDBC("oracle.jdbc.OracleDriver", classPath=drv_path, " ")
  constr <- paste0("jdbc:oracle:thin:@",hostname,":",port,":",sid)
  if (is.na(username)){
  username <- tryCatch(dlgInput("Please enter your AnalysisUserName: ", "AnalysisUserName")$res,
                   error = function(e) {
                   ...
                   warning = function(w) {
                     u <- readline("Please enter your AnalysisUserName: ")
                     return(u) } )}
  con <- dbConnect(drv, constr, username, getPass())
